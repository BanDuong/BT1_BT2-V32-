...
class LeCartCustom(LeBasecart):
	...
	def display_import_source(self):
		...
		queries = {
			...
			'categories': {
				'type': 'select',
				'query': "SELECT COUNT(categories_id) AS count FROM  _DBPRF_categories WHERE categories_id > " + to_str(self._notice['process']['categories']['id_src']),
			},
			'products': {
				'type': 'select',
				'query': "SELECT COUNT(products_id) AS count FROM  _DBPRF_products WHERE products_id > " + to_str(self._notice['process']['products']['id_src']),
			},
			'customers': {
				'type': 'select',
				'query': "SELECT COUNT(customers_id) AS count FROM  _DBPRF_customers WHERE customers_id > " + to_str(self._notice['process']['customers']['id_src']),
			},
			'orders': {
				'type': 'select',
				'query': "SELECT COUNT(orders_id) AS count FROM  _DBPRF_orders WHERE orders_id > " + to_str(self._notice['process']['orders']['id_src']),
			},
			# 'reviews': {
			# 	'type': 'select',
	...
	def convert_category_export(self, categories, categories_ext):
		...
		# category_data['parent'] = parent['parent_id']
		# category_data['active'] = True if categories['active']=='1' else False
		# img = get_row_from_list_by_field(categories_ext['data']['cate_imgs'],'cat_id', categories['categories_id'])
		...
		category_data['created_at'] = categories['date_added'] #datetime.fromtimestamp(to_int(categories['date_added'])) if categories['date_added'] else '0000-00-00 00:00:00'
		category_data['updated_at'] = categories['last_modified'] #datetime.fromtimestamp(to_int(categories['last_modified'])) if categories['last_modified'] else '0000-00-00 00:00:00'
		category_description = get_row_from_list_by_field(categories_ext['data']['categories_description'],'categories_id', categories['categories_id'])
		...
		# category_data['meta_title'] = categories_ext['page_title']
		# category_data['meta_keyword'] = categories_ext['code']
		# meta_description
	...
	def get_products_ext_export(self, products):
		...
		product_ext_queries = {
			...
			'products_to_categories': {
				'type': "select",
				'query': "SELECT * FROM  _DBPRF_products_to_categories WHERE products_id IN " + product_id_con + " ORDER BY products_id ASC ",
			},
			# 'products_meta': {
			# 	'type': "select",
		...
		# self.log(product_ext_queries, 'product_ext_queries')

		product_ext = self.get_connector_data(url_query, {'serialize': True, 'query': json.dumps(product_ext_queries)})
		if (not product_ext) or product_ext['result'] != 'success':
			return response_error()
		# image_ids = duplicate_field_value_from_list(product_ext['data']['products_description'], 'products_name')
		...
		# product_ext_rel = self.get_connector_data(url_query, {'serialize': True, 'query': json.dumps(product_ext_rel_queries),})
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data['created_at'] = product['products_date_added']  #datetime.fromtimestamp(to_int(product['products_date_added'])) if product['dt_created'] else get_current_time()
		product_data['updated_at'] = product['products_last_modified']  #datetime.fromtimestamp(to_int(product['dt_updated'])) if to_int(product['dt_updated']) else product_data['created_at']

		name = get_row_from_list_by_field(products_ext_data['products_description'], 'products_id', product['products_id'])['products_name']
		product_data['name'] = html.unescape(name)

		description = get_row_from_list_by_field(products_ext_data['products_description'], 'products_id', product['products_id'])['products_description']
		product_data['description'] = html.unescape(description)
		...
		#
		#
		# product_images = get_list_from_list_by_field(products_ext_data['products_images'], 'product_id',product['product_id'])
		# if product_images:
		# 	if check_thumbnail:
		...
		if address_books:
			for address_book in address_books:
				...
				if country:
					...
					address_data['country']['id'] = address_book['address_book_id']
	...
	def get_orders_ext_export(self, orders):
		...
		payment_zone = duplicate_field_value_from_list(orders['data'], 'payment_method')
		shipping_zone = duplicate_field_value_from_list(orders['data'], 'shipping_method')
		...
		orders_ext_queries = {
			...
			'address_book': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_address_book"
			},
			# 'orders_customer': {
			# 	'type': 'select',
			...
			# 'orders_payment': {
			# 	'type': 'select',
			# 	'query': "SELECT * FROM  _DBPRF_orders WHERE payment_method IN " + self.list_to_in_condition(state_ids)
			# },
			...
			'countries': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_countries "
			},

			'zones': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_zones"
			}
		}
	...
	def convert_order_export(self, order, orders_ext):
		...
		order_products = get_list_from_list_by_field(orders_ext['data']['orders_total'], 'class', 'ot_shipping')
		for order_pr in order_products:
			order_data['shipping']['amount'] = get_value_by_key_in_dict(order_pr,'value',0.000)
			order_data['shipping']['title'] = get_value_by_key_in_dict(order_pr,'title','Shipping')
		...
		order_products = get_list_from_list_by_field(orders_ext['data']['orders_total'], 'class', 'ot_tax')
		for order_pr in order_products:
			order_data['tax']['title'] = get_value_by_key_in_dict(order_pr,'title','Tax')
			order_data['tax']['amount'] = get_value_by_key_in_dict(order_pr, 'value', 0.0000)
		...
		order_products = get_list_from_list_by_field(orders_ext['data']['orders_total'],'class','ot_total')
		for order_pr in order_products:
			order_data['total']['title'] = get_value_by_key_in_dict(order_pr, 'title','Total')
			order_data['total']['amount'] = get_value_by_key_in_dict(order_pr, 'value', 0.0000)

		order_products = get_list_from_list_by_field(orders_ext['data']['orders_total'], 'class', 'ot_subtotal')
		for order_pr in order_products:
			order_data['subtotal']['title'] = get_value_by_key_in_dict(order_pr, 'title','Sub-total')
			order_data['subtotal']['amount'] = get_value_by_key_in_dict(order_pr, 'value', 0.0000)
		...
		order_customer['id'] = order['orders_id']
		order_customer['email'] = order['customers_email']
		delivery_name = order['delivery_name'] #+ ' ' + order['ship_lname']
		name = delivery_name.split()
		...
		customer_address['last_name'] = name[1]
		...
		phone =order['customers_telephone'].replace('-','')
		if phone.isdigit()==False:
			phone = ''
		customer_address['telephone'] = phone
		customer_address['company'] = order['delivery_company']

		# customer_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_id', order['orders_id'])
		# if customer_country:
		...
		customer_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'zone_country_id', orders_ext['data']['countries'][0]['countries_id'])
		if customer_state:
		...
		billing_name = order['billing_name'] #+ ' '+ order['bill_lname'])
		name2 = billing_name.split()
		order_billing['first_name'] = name2[0]
		order_billing['last_name'] = name2[1]
		...
		order_billing['telephone'] = phone
		order_billing['company'] = order['billing_company']
		# billing_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'orders_id', order['orders_id'])
		# if billing_country:
		...
		billing_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'zone_country_id', orders_ext['data']['countries'][0]['countries_id'])
		if billing_state:
		...
		order_delivery['first_name'] = name[0]
		order_delivery['last_name'] = name[1]
		...
		order_delivery['telephone'] = phone
		order_delivery['company'] = order['delivery_company']

		delivery_country = orders_ext['data']['countries']

		if delivery_country:
			order_delivery['country']['id'] = delivery_country[0]['countries_id']
			order_delivery['country']['code'] = delivery_country[0]['countries_iso_code_2']
			order_delivery['country']['country_code'] = delivery_country[0]['countries_iso_code_3']
			order_delivery['country']['name'] = delivery_country[0]['countries_name']

		delivery_state = orders_ext['data']['zones']
		if delivery_state:
			order_delivery['state']['id'] = delivery_state[0]['zone_id']
			order_delivery['state']['state_code'] = delivery_state[0]['zone_code']
			order_delivery['state']['name'] = delivery_state[0]['zone_name']
		...
		# payments = get_row_from_list_by_field(orders_ext['data']['orders_payment'], 'order_id', order['order_id'])
