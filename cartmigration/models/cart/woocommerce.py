...
class LeCartWoocommerce(LeCartWordpress):
	...
	def category_import(self, convert, category, categories_ext):
		...
		self.insert_map('taxonomy_cate', term_id, term_taxonomy_id)
	...
	def product_import(self, convert, product, products_ext):
		product_post = {
			...
			'post_type': 'product',
			'post_date': product['products_date_added'],
			'post_date_gmt': product['products_date_added'],
			'post_modified': product['products_last_modified'],
			'post_modified_gmt': product['products_last_modified']
		}
...
#-----------------------------------------------------post_meta------------------------------------------------#
		...
		meta_value = [convert['sku'], product['products_msrp'], product['products_wholesale_price'], product['products_wholesale_price']]
...
#------------------------------------------------------image-----------------------------------------------------#
		path = "/opt/lampp/htdocs/customcart/images/"
		img = product['products_image']

		if os.path.isfile(path+img):
			re_path = "http://localhost/wordcart/wp-content/uploads/"
			re_img = str(date.today().year) + '/' + '0' + str(date.today().month) + '/' + img.replace('.png', '.jpg')
			img_data ={
				'post_author': '1',
				'post_content': convert['description'],
				'post_title': img.replace('.png',''),
				'post_status': 'inherit',
				'comment_status': 'open',
				'ping_status': 'closed',
				'post_name': img.replace('.png','').lower(),
				'post_date': product['products_date_added'],
				'post_date_gmt': product['products_date_added'],
				'post_modified': product['products_last_modified'],
				'post_modified_gmt': product['products_last_modified'],
			...
			img_query = self.create_insert_query_connector('posts',img_data)
			img_id = self.import_data_connector(img_query,'image')
			...
			img_meta_query = self.create_insert_query_connector('postmeta', img_mt)
			img_meta_id = self.import_data_connector(img_meta_query, 'image_meta')
			...
			img_meta_id = self.import_data_connector(img_meta_query, 'image_meta')
#-----------------------------------------------products_to_cates------------------------------------------------#
		category_id = get_list_from_list_by_field(products_ext['data']['products_to_categories'], 'products_id', convert['id'])[0]['categories_id']
		term_taxonomy_id = self.get_map_field_by_src('taxonomy_cate', self.get_map_field_by_src(self.TYPE_CATEGORY, category_id))
		term_relationship = {
			'object_id': ID,
...
#-------------------------------------------meta_lookup------------------------------------------------------#
	...
	def customer_import(self, convert, customer, customers_ext):
		user_data ={
			'user_login': convert['first_name'],
			'user_pass': convert['password'],
			'user_nicename': convert['first_name'],
			'user_email': convert['email'],
			'user_url': 'http://localhost/wordcart',
			'user_registered': convert['created_at'],
			# 'user_activation_key': ,
			# 'user_status': ,
			'display_name': convert['first_name'] + ' ' + convert['last_name']
		}
		user_query = self.create_insert_query_connector('users',user_data)
		user_id = self.import_data_connector(user_query,'users')
		self.insert_map(self.TYPE_CUSTOMER,customer['customers_id'],user_id)

		address_books = get_list_from_list_by_field(customers_ext['data']['address_book'], 'customers_id',customer['customers_id'])
		country = get_row_from_list_by_field(customers_ext['data']['countries'], 'countries_id', address_books[0]['entry_country_id'])
		user_meta_data ={
			'nickname': convert['first_name'],
			...
			'billing_company': address_books[0]['entry_company'],
			'billing_address_1': address_books[0]['entry_street_address'],
			'billing_address_2': '',
			'billing_city': address_books[0]['entry_city'],
			'billing_postcode': address_books[0]['entry_postcode'],
			'billing_country': country,
			'billing_state': address_books[0]['entry_state'],
		...
		for key,value in user_meta_data.items():
			user_meta ={
				'user_id': user_id,
				'meta_key': key,
				'meta_value': value
			}
			user_meta_query = self.create_insert_query_connector('usermeta',user_meta)
			user_meta_id = self.import_data_connector(user_meta_query,'user_meta')

		cus_ext_data = get_list_from_list_by_field(customers_ext['data']['address_book'],'customers_id',customer['customers_id'])
		customer_lookup_data = {
			'user_id': user_id,
			'username': convert['first_name'],
			'first_name': convert['first_name'],
			'last_name': convert['last_name'],
			'email': convert['email'],
			...
			'postcode': cus_ext_data[0]['entry_postcode'],
			'city': cus_ext_data[0]['entry_city'],
			'state': cus_ext_data[0]['entry_state'],
		}
		customer_lookup_query = self.create_insert_query_connector('wc_customer_lookup',customer_lookup_data)
		customer_id = self.import_data_connector(customer_lookup_query,'customer_lookup')

		mailpoet_subscribers_data ={
			'wp_user_id': user_id,
			'first_name': convert['first_name'],
			'last_name': convert['last_name'],
			'email': convert['email'],
		...
		mailpoet_subscribers_query = self.create_insert_query_connector('mailpoet_subscribers',mailpoet_subscribers_data)
		mailpoet_subscribers_id = self.import_data_connector(mailpoet_subscribers_query,'subscribers')
	...
	def order_import(self, convert, order, orders_ext):
		...
		post_query = self.create_insert_query_connector('posts', post_data)
		order_id = self.import_data_connector(post_query, 'order_id')
...
#-----------------------------------shipping---------------------------------------------------------#
		...
		ship_query = self.create_insert_query_connector('woocommerce_order_items',ship_data)
		ship_id = self.import_data_connector(ship_query,'shipping')

		ship_meta ={
			'cost': convert['shipping']['amount'],
			'total_tax': convert['tax']['amount']
		}
		total+= float(ship_meta['cost'])
		for key,value in ship_meta.items():
			ship_meta_data ={
				'order_item_id': ship_id,
				'meta_key': key,
				'meta_value': value
			}
			ship_meta_query = self.create_insert_query_connector('woocommerce_order_itemmeta',ship_meta_data)
			self.import_data_connector(ship_meta_query,'ship_meta')
# -----------------------------------fee---------------------------------------------------------#
		fee_data = {
			'order_item_name': 'Fee',
			'order_item_type': 'fee',
		...
		fee_query = self.create_insert_query_connector('woocommerce_order_items', fee_data)
		fee_id = self.import_data_connector(fee_query, 'shipping')

		fee_meta = {
			'_fee_amount': convert['tax']['amount'],
			'_tax_status': 'taxable',
			'_line_total': convert['tax']['amount'],
			'_line_tax': convert['tax']['amount'],
		}
		total+= float(fee_meta['_fee_amount'])
		for key, value in fee_meta.items():
			fee_meta_data = {
				'order_item_id': fee_id,
				'meta_key': key,
				'meta_value': value
			}
			fee_meta_query = self.create_insert_query_connector('woocommerce_order_itemmeta', fee_meta_data)
			self.import_data_connector(fee_meta_query, 'ship_meta')
# -----------------------------------order_items---------------------------------------------------------#
		...
		for order_product in order_products:
			...
			items_query = self.create_insert_query_connector('woocommerce_order_items',items_data)
			items_id = self.import_data_connector(items_query,'order_items')
#----------------------------------order_items_meta----------------------------------------------------------#
			items_mate_data ={
				# '_product_id': ,
				'_variation_id': 0,
				...
				'_tax_class': '',
				'_line_subtotal': convert['subtotal']['amount'],
				'_line_subtotal_tax': convert['tax']['amount'],
				'_line_total': convert['total']['amount'],
				'_line_tax': convert['tax']['amount'],
			}
			total+= float(items_mate_data['_line_total'])
			for key,value in items_mate_data.items():
				item_meta ={
					'order_item_id': items_id,
					'meta_key': key,
					'meta_value': value
				}
				item_meta_query = self.create_insert_query_connector('woocommerce_order_itemmeta',item_meta)
				self.import_data_connector(item_meta_query,'items_meta')
# ---------------------------------------------------post_meta-----------------------------------------------#
		postmeta_data = {
			'_edit_last': 1,
			# '_customer_user': ,
			...
			'_billing_first_name': order['billing_name'].split()[0],
			'_billing_last_name': order['billing_name'].split()[1],
			'_billing_address_1': order['billing_address1'],
			'_billing_city': order['billing_city'],
			'_billing_postcode': order['billing_postcode'],
			'_billing_email': '',  # NULL
			'_billing_phone': order['customers_telephone'],
			'_billing_country': order['billing_country'],
			'_shipping_first_name': order['delivery_name'].split()[0],
			'_shipping_last_name': order['delivery_name'].split()[1],
			'_shipping_company': order['delivery_company'],
			'_shipping_address_1': order['delivery_address1'],
			...
			'_shipping_country': order['delivery_country'],
		}
		for key, value in postmeta_data.items():
			postmeta_data = {
				'post_id': order_id,
				'meta_key': key,
				'meta_value': value
			}
			postmeta_query = self.create_insert_query_connector('postmeta', postmeta_data)
			self.import_data_connector(postmeta_query, 'postmeta')
# ---------------------------------order_stats---------------------------------------------------------#
# 			stats_data = {
# 				'order_id': order_id,
# 				'date_created': order['date_purchased'],
# 				'date_created_gmt': order['date_purchased'],
# 				'num_items_sold': order_product['products_quantity'],
# 				'total_sales': convert['total']['amount'],
# 				'shipping_total': convert['shipping']['amount'],
# 				'tax_total': convert['tax']['amount'],
# 				'net_total': convert['total']['amount'] - convert['shipping']['amount'] - convert['tax']['amount'],
# 				'status': 'wc_pending'
# 				# 'customer_id':
# 			}
# 			stats_query = self.create_insert_query_connector('wc_order_stats', stats_data)
# 			self.import_data_connector(stats_query, 'stats')
